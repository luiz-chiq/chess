import { BoardCollumn, BoardPosition, BoardRow } from "../types/types";
...
export function getBoardPositionByIndexes(row: number, col: number): BoardPosition {
    return getCol(col) + getRow(row) as BoardPosition;
}

export function getRowIndex(position: BoardPosition): number {
    return parseInt(position[1]) - 1;
}

export function getColIndex(position: BoardPosition): number {
