...
function getPawnMoveset(
  ...
  const canTakeRightDiagonal =
    rightAvailable &&
    chessBoard[rowIndex + direction][colIndex + 1].piece
      ?.color === oppositeColor;
...
function getRookMoveset(
  ...
  while (rowUp < 7) {
    rowUp += 1;
    const position = getBoardPositionByIndexes(
      rowUp,
      colIndex
    );
    if (
      chessBoard[rowUp][colIndex].piece?.color ===
      piece.color
    )
      break;
  ...
  while (rowDown > 0) {
    ...
    if (
      chessBoard[rowDown][colIndex].piece?.color ===
      piece.color
    )
      break;
  ...
  while (colRight < 7) {
    ...
    if (
      chessBoard[rowIndex][colRight].piece?.color ===
      piece.color
    )
      break;
  ...
  while (colLeft > 0) {
    ...
    if (board.getPiece(position)?.color === piece.color)
      break;
...
function getBishopMoveset(
  ...
  const upRightValidation = (
    i: number,
    j: number
  ): boolean => i < 7 && j < 7;
  const upLeftValidation = (
    i: number,
    j: number
  ): boolean => i < 7 && j > 0;
  const downRightValidation = (
    i: number,
    j: number
  ): boolean => i > 0 && j < 7;
  const downLeftValidation = (
    i: number,
    j: number
  ): boolean => i > 0 && j > 0;

  const getDiagonalMoveset = (
...
function getKingMoveset(
  ...
  const opponentKingPosition =
    board.getKingPosition(oppositeColor);
  const opponentKingRowIndex = getRowIndex(
    opponentKingPosition
  );
  const opponentKingColIndex = getColIndex(
    opponentKingPosition
  );

  const circle = getKingMovementCircle(rowIndex, colIndex);

  const opponentKingCircle = getKingMovementCircle(
    opponentKingRowIndex,
    opponentKingColIndex
  );

  circle.filter((position) =>
    opponentKingCircle.includes(position)
  );
...
function getKingMovementCircle(
  rowIndex: number,
  colIndex: number
): BoardPosition[] {
  ...
  if (upAvailable)
    circle.push(
      getBoardPositionByIndexes(rowIndex + 1, colIndex)
    );
  if (rightAvailable && upAvailable)
    circle.push(
      getBoardPositionByIndexes(rowIndex + 1, colIndex + 1)
    );
  if (rightAvailable)
    circle.push(
      getBoardPositionByIndexes(rowIndex, colIndex + 1)
    );
